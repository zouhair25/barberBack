...
public class StatisticsController {
    ...
    public ResponseEntity<List<Map<String, Object>>> revenue(
        ...
        return ResponseEntity.ok(statisticsService.getRevenueByPeriod(principal.getUserCentreSoins().getFirst().getId(), from, to, groupBy));
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> topServices(
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime to,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(statisticsService.getTopServices(principal.getUserCentreSoins().getFirst().getId(), from, to));
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> loyalClients(
            ...
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(statisticsService.getLoyalClients(principal.getUserCentreSoins().getFirst().getId(), limit));
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> topProducts(
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime to,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(statisticsService.getTopProducts(principal.getUserCentreSoins().getFirst().getId(), from, to));
    }
}
