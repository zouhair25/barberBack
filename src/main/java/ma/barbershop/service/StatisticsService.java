...
public class StatisticsService {
    ...
    @Transactional(readOnly = true)
    public List<Map<String, Object>> getRevenueByPeriod(Long userCentreSoinId, LocalDateTime from, LocalDateTime to, String groupBy) {
        ...
        String sql = """
                SELECT DATE_TRUNC('%s', i.issued_at) AS period,
                       ...
                       COUNT(i.id) AS count
                FROM invoices i
                WHERE i.user_centre_soin_id = ? AND i.issued_at BETWEEN ? AND ?
                GROUP BY period
                ORDER BY period ASC
                """.formatted(truncFunc);

        return jdbcTemplate.queryForList(sql, userCentreSoinId,
                java.sql.Timestamp.valueOf(from),
                java.sql.Timestamp.valueOf(to));
    }

    @Transactional(readOnly = true)
    public List<Map<String, Object>> getTopServices(Long userCentreSoinId, LocalDateTime from, LocalDateTime to) {
        String sql = """
                ...
                WHERE i.user_centre_soin_id = ? AND i.issued_at BETWEEN ? AND ? AND il.line_type = 'SERVICE'
    ...
    @Transactional(readOnly = true)
    public List<Map<String, Object>> getLoyalClients(Long userCentreSoinId, int limit) {
        String sql = """
                ...
                WHERE a.user_centre_soin_id = ? AND a.status = 'COMPLETED'
                GROUP BY u.id, u.first_name, u.last_name, u.email, u.phone
                ORDER BY visit_count DESC
                LIMIT ?
                """;

        return jdbcTemplate.queryForList(sql, userCentreSoinId, limit);
    }

    @Transactional(readOnly = true)
    public List<Map<String, Object>> getTopProducts(Long userCentreSoinId, LocalDateTime from, LocalDateTime to) {
        String sql = """
                SELECT p.name, p.category, SUM(il.quantity) AS quantity_sold, SUM(il.total_price) AS revenue
                FROM invoice_lines il
                JOIN products p ON il.product_id = p.id
                JOIN invoices i ON il.invoice_id = i.id
                WHERE i.user_centre_soin_id = ? AND i.issued_at BETWEEN ? AND ? AND il.line_type = 'PRODUCT'
